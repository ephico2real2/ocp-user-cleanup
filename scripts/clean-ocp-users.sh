...
delete_users() {
    ...
    local total=0
    total=$(wc -l < "$CSV_FILE" | tr -d ' ') || total=0
    if [ "$total" -gt 0 ]; then
    ...
    # Get excluded count
    local excluded_count=0
    excluded_count=$(grep -c ",true$" "$CSV_FILE" 2>/dev/null) || excluded_count=0
    
    # Calculate actionable users
    local actionable=0
    if [ "$total" -ge "$excluded_count" ]; then
        actionable=$((total - excluded_count))
    fi
